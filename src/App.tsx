...
export default function App() {
  ...
  const getScore = (ri: number, ci: number): Score =>
    scores[`${ri}-${ci}`] || { s1: "", s2: "", done: false };

  const setScore = (ri: number, ci: number, field: keyof Score, val: string | boolean) => {
    const key = `${ri}-${ci}`;
    setScores(prev => ({ ...prev, [key]: { ...getScore(ri, ci), [field]: val } }));
  };

  const submitScore = (ri: number, ci: number) => {
  ...
  const editScore = (ri: number, ci: number) => {
